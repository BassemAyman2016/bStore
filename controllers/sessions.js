...
const currentConfig = require('../config/setup')
const crypto = require('crypto')
const fs = require('fs')
const EmailAdapter = require('../helpers/mailAdapter')
const path  = require('path')
require('dotenv').config();
const tokenKey = currentConfig.secretOrKey
            ...
            if(!checkIfCustomer){
                ...
                if(match){
                    if(checkIfCustomer.deleted){
                        ...
                        var res
                        return res.status(200).send({ status: 'success', token: `bearer ${token}`, type:'customer', id: checkIfCustomer.id, name:checkIfCustomer.first_name+" "+checkIfCustomer.last_name })
